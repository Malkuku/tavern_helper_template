  ...
  <div class="data-table">
    ...
    <div v-if="rows.length" class="table-list">
      ...
      <div v-for="row in pageRows" :key="row.k" class="table-row">
        <!-- 勾选框：只在删除模式下出现 -->
        <input v-if="deleteMode" v-model="checkedKeys" type="checkbox" :value="row.k" class="row-check" />
    ...
    <div v-if="totalPage > 1" class="pagination">
      <button :disabled="current === 1" @click="current--">‹</button>

      <button v-for="p in displayPages" :key="p" :class="{ active: p === current }" @click="current = p">
        {{ p }}
      </button>

      <button :disabled="current === totalPage" @click="current++">›</button>
    </div>
  </div>
</template>
