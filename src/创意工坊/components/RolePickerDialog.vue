  ...
  <div v-if="open" class="picker-backdrop" @click.self="cancel">
    <section class="picker" role="dialog" aria-modal="true" aria-labelledby="role-picker-title">
      <header>
        <div>
          <p>CAST LIBRARY</p>
          <h2 id="role-picker-title">组建角色阵容</h2>
        </div>
      ...
      <div class="picker-body">
        ...
        <aside v-if="focused" class="detail">
          <div class="portrait">
            <RoleAvatar
              class="detail-avatar"
              :src="focused.items[0].entry.meta?.avatar"
              :alt="focused.title"
              :seed="focused.items[0].entry.key"
              :fallback-style="focused.items[0].entry.meta?.avatarStyle"
              :theme-color="focused.items[0].entry.meta?.color"
            />
            <div>
              <p>{{ focused.type }}</p>
              <h3>{{ focused.title }}</h3>
              <span>{{ focused.summary }}</span>
            </div>
          </div>
          <h4>选择版本</h4>
          <button
          ...
          <button
            ...
            type="button"
            class="remove"
            @click="removeIdentity(focused.identity)"
          >
            从阵容移除
          </button>
        </aside>
      ...
      <footer>
        <div class="selection">
          <span>当前阵容</span><b>{{ selected.length }} 人</b><small>{{ selectedNames }}</small>
        </div>
        <button type="button" @click="cancel">取消</button
        ><button type="button" class="primary" @click="confirm">确认阵容</button>
      </footer>
    </section>
  </div>
</template>
...
import { normalizeRoleSelection, roleIdentityOf } from '../assets/model';
...
const groups = computed(() => {
  ...
  return [...map.entries()].map(([identity, items]) => ({
    ...
    title: assetTitle('角色', items[0].entry),
    summary: roleSummary(items[0].entry),
    terms: items
      .map(item => `${assetTitle('角色', item.entry)} ${item.entry.author} ${item.entry.desc}`)
      .join(' ')
      .toLowerCase(),
  }));
});
const filteredGroups = computed(() =>
  groups.value.filter(
    group => (!type.value || group.type === type.value) && group.terms.includes(query.value.toLowerCase()),
  ),
);
const focused = computed(() => groups.value.find(group => group.identity === focusedIdentity.value));
const selectedNames = computed(
  () => selected.value.map(id => assetTitle('角色', props.source.registries.角色[id])).join('、') || '尚未选择',
);
watch(
  () => props.open,
  open => {
    ...
    const first = selected.value[0]
      ? roleIdentityOf(props.source.registries.角色[selected.value[0]])
      : groups.value[0]?.identity;
...
function choose(id: string) {
  ...
  selected.value = [
    ...selected.value.filter(current => roleIdentityOf(props.source.registries.角色[current]) !== identity),
    id,
  ];
}
function removeIdentity(identity: string) {
  selected.value = selected.value.filter(id => roleIdentityOf(props.source.registries.角色[id]) !== identity);
}
function cancel() {
  emit('cancel');
}
