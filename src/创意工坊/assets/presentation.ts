...
export function defaultRoleData(type: string): JsonObject {
  ...
  if (type === '次要角色') return { 姓名: '', 名称检索词: [], 区域检索词: [], 在场: false, 简介: '', 性格标签: [], ...common };
  const shared = { 年龄: '', 当前身份: '', 外貌: [], 背景: [], 性格: { 社交表现: '', 行动逻辑: '', 思维习惯: '', 人际距离: '', 道德底色: '' }, 人际关系: {}, 性经验: {} };
  if (type === '主要角色') return { 姓名: '', 名称检索词: [], 区域检索词: [], 在场: false, ...shared, 当前想法: '', 外貌概括: '', 战斗风格: [], 语料: {}, ...common };
...
export function diffSources(before: ScenarioSourceBundle, after: ScenarioSourceBundle): AssetChange[] {
  const left = assetsOf(before), right = assetsOf(after), changes: AssetChange[] = [];
  for (const category of workshopCategories) {
    for (const id of new Set([...Object.keys(left[category]), ...Object.keys(right[category])])) {
      const oldValue = left[category][id], newValue = right[category][id];
      if (oldValue === undefined) changes.push({ category, id, kind: 'added', title: assetTitle(category, newValue), fields: flattenDiff(undefined, newValue) });
      else if (newValue === undefined) changes.push({ category, id, kind: 'removed', title: assetTitle(category, oldValue), fields: flattenDiff(oldValue, undefined) });
      else { const fields = flattenDiff(oldValue, newValue); if (fields.length) changes.push({ category, id, kind: 'modified', title: assetTitle(category, newValue), fields }); }
    }
  }
  return changes;
}
...
function flattenDiff(oldValue: unknown, newValue: unknown, path = ''): FieldChange[] {
  ...
  if (isRecord(oldValue) && isRecord(newValue)) return [...new Set([...Object.keys(oldValue), ...Object.keys(newValue)])].flatMap(key => flattenDiff(oldValue[key], newValue[key], path ? `${path}.${key}` : key));
...
export function previewPackage(source: ScenarioSourceBundle, pkg: WorkshopPackage): ImportPreview {
  ...
  const merged = klona(source); const target = assetsOf(merged); for (const [category, entries] of Object.entries(pkg.assets) as [WorkshopCategory, Record<string, unknown>][]) Object.assign(target[category], klona(entries));
