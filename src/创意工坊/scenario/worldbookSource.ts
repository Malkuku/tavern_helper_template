...
import { CollectionEntrySchema, MapEntrySchema, ScenarioEntrySchema, TypedCollectionEntrySchema } from './schemas';
...
function parseRoot<T>(entries: ConfigEntry[], category: SourceCategory, schema: z.ZodType<T>): T {
  ...
  if (!result.success) {
    throw new ScenarioDataError(
      'INVALID_DOCUMENT',
      `配置条目结构无效：${scenarioWorldbookEntryNames[category]}\n${z.prettifyError(result.error)}`,
      {
        category,
        cause: result.error,
      },
...
export function parseScenarioSourceEntries(entries: ConfigEntry[]): ScenarioSourceBundle {
  if (entries.some(entry => entry.name === '<地图节点>配置')) {
    throw new ScenarioDataError('INVALID_DOCUMENT', '不支持旧地图节点配置；请提供 data 完整嵌套地图。', {
      category: '地图',
    });
  }
  ...
  const mapDocument = z
    .object({ 地图: z.record(z.string(), z.unknown()) })
    .strict()
    .parse(rawMaps);
